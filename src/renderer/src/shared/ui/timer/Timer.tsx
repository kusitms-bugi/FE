...
type Props = {
  ...
  size?: number; // 기본 64
  on?: string; // 하이라이트 색 (기본 yellow-500)
...
const DEFAULT_OFF = 'var(--color-grey-50)';

// 배경 대비 세그먼트+숫자 확대 비율
const SEG_SCALE = 1.6;
// 세그먼트+숫자를 살짝 위로 올리기 위한 오프셋 (뷰박스 기준)
const SEG_Y_OFFSET = -4; // 값 더 줄이면 위로 더 올라감
...
const Timer = function Timer({
  ...
  const centerD = centerPathMap[value];
  ...
  return (
    ...
    >
      <g clipPath="url(#clip0)">
        {/* 흰 동그라미 배경 */}
        <circle cx="50" cy="50" r="50" fill="var(--color-grey-0)" />

        {/* 세그먼트 + 숫자 그룹: 확대 + 위로 살짝 이동 */}
        <g
          transform={`
            translate(50 50)
            scale(${SEG_SCALE})
            translate(-50 ${-50 + SEG_Y_OFFSET})
          `}
        >
          {/* 세그먼트들 */}
          <path
