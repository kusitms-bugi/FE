...
>(({ checked, onChange, uncheckedLabel, checkedLabel }, ref) => {
    ...
    return (
        <button
            ...
            className={cn(
                'bg-grey-25 relative flex h-[28px] w-fit items-center gap-2 rounded-full px-[12px] cursor-pointer text-caption-xs-meidum',
            )}
        >
            <span
                className={cn(
                    'absolute left-[12px] h-[22px] rounded-full bg-grey-0 transition-all duration-400 ease-in-out',
                )}
                style={{
